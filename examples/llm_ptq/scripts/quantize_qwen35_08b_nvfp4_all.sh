...
# 用法: bash examples/llm_ptq/scripts/quantize_qwen35_08b_nvfp4_all.sh
...
# - 仍然有两类模块保留 BF16: 非 Linear/Conv 矩阵 (LayerNorm/Embedding) 和 bias
...
DST_MODEL="/data/disk1/guohaoran/models/Qwen3.5-0.8B-NVFP4-ALL-T1"
...
if [ -f "$VENV/bin/activate" ]; then
    ...
    echo "quantize_qwen35_08b_nvfp4_all.sh: venv not found at $VENV" >&2
...
if [ ! -f "$CALIB_JSONL" ]; then
    echo "quantize_qwen35_08b_nvfp4_all.sh: generating calibration JSONL at $CALIB_JSONL"
...
PY
    ...
    echo "quantize_qwen35_08b_nvfp4_all.sh: reusing existing calibration JSONL: $CALIB_JSONL"
fi
...
# ============================================================================
mkdir -p "$LOG_DIR"
echo "quantize_qwen35_08b_nvfp4_all.sh: quantizing $SRC_MODEL -> $DST_MODEL"
echo "quantize_qwen35_08b_nvfp4_all.sh: log -> $LOG_FILE"
...
echo "quantize_qwen35_08b_nvfp4_all.sh: done. output -> $DST_MODEL"
du -sh "$DST_MODEL"
